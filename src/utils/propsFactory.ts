...
export function propsFactory<PropsOptions extends ComponentObjectPropsOptions>(
  ...
  return <Defaults extends PartialKeys<PropsOptions> = object>(
    ...
    // Код взят из библиотеки, поэтому отключу правило
...
type InferPropType<T> = [T] extends [null]
  ...
  : [T] extends [{ type: null | true }]
    ...
    : [T] extends [{ type: ObjectConstructor } | ObjectConstructor]
      ? Record<string, any>
      : [T] extends [{ type: BooleanConstructor } | BooleanConstructor]
        ? boolean
        : [T] extends [{ type: DateConstructor } | DateConstructor]
          ? Date
